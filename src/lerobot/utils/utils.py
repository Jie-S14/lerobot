...
def get_cartesian_from_polar(radian, radius, x0: float = 0.0, y0: float = 0.0):
    # x = x0 + radius * np.cos(angle)
    # y = y0 + radius * np.sin(angle) # not fit isaac sim coordinate
    x = x0 + radius * np.sin(radian)
    y = y0 + radius * np.cos(radian)
    return x, y
